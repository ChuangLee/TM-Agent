...
describe("tmux parser", () => {
  ...
  test("parses windows and panes", () => {
    ...
    const panes = parsePanes("0\t%1\tbash\t1\t120x30\t1\t/home/u/proj");
  ...
  test("parsePanes defaults currentPath to empty when tmux omits it", () => {
    const panes = parsePanes("0\t%1\tbash\t1\t120x30\t0");
    expect(panes[0].currentPath).toBe("");
  });
});
