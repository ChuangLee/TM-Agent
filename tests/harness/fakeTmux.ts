...
export class FakeTmuxGateway implements TmuxGateway {
  ...
  /**
   * Test helper: mutate the active pane's `command` (what tmux reports as
   * `#{pane_current_command}`). Used by e2e specs to simulate the user
   * launching vim / claude / less / python etc. without wiring a real shell.
   */
  public setActivePaneCommand(sessionName: string, command: string): void {
    const session = this.findSession(sessionName);
    const window = session.windows.find((w) => w.active) ?? session.windows[0];
    const pane = window?.panes.find((p) => p.active) ?? window?.panes[0];
    if (pane) pane.command = command;
  }

  /**
   * Test helper: mutate the active pane's `#{pane_current_path}`. File-panel
   * tests use this to simulate the user running `cd` inside the shell so the
   * FilePanel's root follows.
   */
  public setActivePanePath(sessionName: string, cwd: string): void {
    const session = this.findSession(sessionName);
    const window = session.windows.find((w) => w.active) ?? session.windows[0];
    const pane = window?.panes.find((p) => p.active) ?? window?.panes[0];
    if (pane) pane.currentPath = cwd;
  }
  ...
  public async createSession(
    ...
    const cwdSuffix = options.cwd ? `:${options.cwd}` : "";
    const cmdSuffix = options.startupCommand ? `|${options.startupCommand}` : "";
    this.calls.push(`createSession:${name}${cwdSuffix}${cmdSuffix}`);
  ...
  public async renameSession(oldName: string, newName: string): Promise<void> {
    this.calls.push(`renameSession:${oldName}:${newName}`);
    const session = this.findSession(oldName);
    if (this.sessions.some((s) => s.name === newName)) {
  ...
  public async capturePane(
    ...
    this.calls.push(
      `capturePane:${paneId}:${lines}:${includeEscapes ? "e" : "raw"}${historyOnly ? ":hist" : ""}`
    );
    const header = `captured ${lines} lines for ${paneId}${includeEscapes ? " (escapes)" : ""}${historyOnly ? " [history]" : ""}`;
    const body = Array.from({ length: 40 }, (_, i) => `scrollback-line-${i}`);
    return [header, ...body].join("\n");
  }

  public async sendKeys(target: string, text: string): Promise<void> {
    this.calls.push(`sendKeys:${target}:${text}`);
  }
