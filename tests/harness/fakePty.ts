...
import type { PtyFactory, PtyProcess, PtySpawnOptions } from "../../src/backend/pty/pty-adapter.js";
...
export class FakePtyFactory implements PtyFactory {
  ...
  public readonly spawnDimensions: Array<{ cols: number; rows: number }> = [];

  public spawnTmuxAttach(session: string, options: PtySpawnOptions): PtyProcess {
    this.lastSpawnedSession = session;
    this.spawnDimensions.push({ cols: options.cols, rows: options.rows });
