...
import type { PtyFactory, PtyProcess, PtySpawnOptions } from "./pty-adapter.js";
import { toFlatStringEnv, withoutTmuxEnv } from "../util/env.js";

const require = createRequire(import.meta.url);
const shellQuote = (value: string): string => `'${value.replaceAll("'", "'\"'\"'")}'`;

const ensureNodePtySpawnHelperExecutable = (logger?: Pick<Console, "log" | "error">): void => {
...
export class NodePtyFactory implements PtyFactory {
  ...
  public constructor(private readonly logger?: Pick<Console, "log" | "error">) {
    this.forceScriptFallback = process.env.TM_AGENT_FORCE_SCRIPT_PTY === "1";
    ensureNodePtySpawnHelperExecutable(this.logger);
  }

  public spawnTmuxAttach(session: string, options: PtySpawnOptions): PtyProcess {
    ...
    try {
      ...
      this.logger?.log("[pty] spawn", shell, args.join(" "), `${options.cols}x${options.rows}`);

      // Spawn at the frontend's measured dimensions so tmux's WINCH handling
      // and any immediate capture-pane (scrollback seed) see the correct pane
      // geometry — previously this was hardcoded 80x24 and tmux would redraw
      // onto the wrong rows once the client arrived at its real size.
