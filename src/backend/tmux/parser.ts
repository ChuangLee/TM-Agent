import type { TmuxPaneState, TmuxSessionSummary, TmuxWindowState } from "../../shared/protocol.js";
...
export const parseSessions = (raw: string): TmuxSessionSummary[] =>
  raw
    ...
    .filter(Boolean)
    .map((line) => {
      const [name, attached, windows, lastActivity] = splitLine(line);
      const activityNum = lastActivity ? Number.parseInt(lastActivity, 10) : NaN;
      return {
        name,
        attached: attached === "1",
        windows: Number.parseInt(windows, 10),
        ...(Number.isFinite(activityNum) ? { lastActivity: activityNum } : {})
      };
    });
